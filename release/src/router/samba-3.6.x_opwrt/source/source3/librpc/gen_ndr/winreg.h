...
struct KeySecurityData {
	uint8_t *data;/* [size_is(size),unique,length_is(len)] */
	uint32_t size;
	uint32_t len;
};
...
struct winreg_StringBuf {
	...
	uint16_t size;
	const char *name;/* [length_is(length/2),unique,charset(UTF16),size_is(size/2)] */
};

struct winreg_ValNameBuf {
	uint16_t length;/* [value(strlen_m_term(name)*2)] */
	uint16_t size;
	const char *name;/* [charset(UTF16),size_is(size/2),length_is(length/2),unique] */
};
...
struct winreg_QueryValue {
	struct {
		...
		enum winreg_Type *type;/* [unique] */
		uint8_t *data;/* [length_is(data_length?*data_length:0),unique,range(0,0x4000000),size_is(data_size?*data_size:0)] */
