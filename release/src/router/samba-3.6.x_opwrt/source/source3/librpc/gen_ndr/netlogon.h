...
struct netr_USER_PRIVATE_INFO {
	...
	uint32_t DataLength;
	uint8_t *SensitiveData;/* [flag(LIBNDR_PRINT_ARRAY_HEX),unique,size_is(DataLength)] */
};
...
struct netr_CIPHER_VALUE {
	uint32_t len;
	uint32_t maxlen;
	uint8_t *cipher_data;/* [length_is(len),unique,size_is(maxlen)] */
};
...
union netr_DELTA_ID_UNION {
	...
	const char *name;/* [unique,case(NETR_DELTA_SECRET),charset(UTF16)] */
}/* [switch_type(netr_DeltaEnum)] */;
...
struct netr_OsVersionInfoEx {
	...
	uint32_t PlatformId;
	const char * CSDVersion;/* [subcontext(0),subcontext_size(256),flag(LIBNDR_FLAG_STR_NULLTERM|LIBNDR_FLAG_ALIGN2)] */
